...
final GoRouter appRouter = GoRouter(
  redirect: (BuildContext context, GoRouterState state) {
    ...
    final onboardingCompleted =
        (settings.get(HiveKeys.onboardingCompleted, defaultValue: false) as bool?) ?? false;
    final onWelcomeRoute = state.matchedLocation == '/welcome';

    if (!onboardingCompleted && !onWelcomeRoute) {
      return '/welcome';
    }
    if (onboardingCompleted && onWelcomeRoute) {
      return '/';
